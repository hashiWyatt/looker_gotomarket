view: tfe_salesforce_customer_acv_monthly {
  derived_table: {
    sql: with

            tfe_salesforce_customers_monthly as (
              select
                distinct
                  date_trunc('month', day) as month,
                  customer_id,
                  max(acv) as acv
              from ${tfe_salesforce_bookings.SQL_TABLE_NAME}
              group by 1, 2
              order by customer_id, date_trunc('month', day)
            ),
            tfe_salesforce_net_new as (
              select
                  month,
                  customer_id,
                  acv,
                  lag(acv, 1) over (partition by customer_id order by month) as prev_acv
                from tfe_salesforce_customers_monthly order by customer_id, month
            )
