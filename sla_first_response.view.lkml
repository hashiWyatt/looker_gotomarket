view: sla_first_response {
  derived_table: {
    sql: Select
      ...
      WHERE ((((leads.created_date ) >= ((DATEADD(day,-89, DATE_TRUNC('day',GETDATE()) ))) AND (leads.created_date ) < ((DATEADD(day,90, DATEADD(day,-89, DATE_TRUNC('day',GETDATE()) ) )))))) AND
      (tasks.subject LIKE 'Reply%')
